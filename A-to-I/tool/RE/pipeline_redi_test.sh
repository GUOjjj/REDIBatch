...
### The arguments need to be modified, included:
### 1. Dependencies:environment, software...
###     You can configure dependencies to prevent error reporting in three ways.
###     a) Write `#!` at the very beginnig of the script.
###     b) Use `export PATH=".../bin:$PATH"`.
###     c) Use variables to declare the path to the software and call the tool with the variable name
###         e.g. py27=".../bin/python2.7"
###              ${py27} hello.py
### 2. Input, Output files url
echo '
# perl5
...
# rediportal
redi_db_url="/hwfssz5/ST_EARTH/P18H19700N0356/guosijia/database/REDIportal/REDIportalV2.0/AnnotatedPositions/TABLE1_${ref_no}.txt.gz"
' >>$7
##########################################################################################
##########################################################################################
...
calculate(){
    echo claculate_start && echo overall_known_geneAA_type_func && ${py37} ${REDIcalculator} ${REDI_table_url} ${redi_db_url} && echo calculate_end
}
...
if [[ ${sra_file} =~ ".sra" ]]
then
    sra2fq && renameFq && catFq && bamProcess_PE && REDI_blat_REDI2 && calculate && adarExp_PE
else
    echo sra_dir_is_fq_file! && cpFq && renameFq && catFq && bamProcess_PE && REDI_blat_REDI2 && calculate && adarExp_PE
fi
