...
projtect_url="[your_url]/[your_project_name]"
...
do 
    ...
    do
        ...
        for SampleID in `awk -F, '{if (NR>1) {print $1}}' ${SraRunTable_csv}|cut -d '_' -f 1|sort|uniq`
            ...
            then
                rm ${SampleID_1_fq}.gz && echo ${SampleID_1_fq}.gz_removed
            fi
            if test -e "${SampleID_2_fq}.gz"
            then
                rm ${SampleID_2_fq}.gz && echo ${SampleID_2_fq}.gz_removed
            fi
            if test -e "${SampleID_1_fq}"
            then
                rm ${SampleID_1_fq} && echo ${SampleID_1_fq}_removed
            fi
            if test -e "${SampleID_2_fq}"
            then
                rm ${SampleID_2_fq} && echo ${SampleID_2_fq}_removed
            fi
            ...
            for SRR in $(awk -F, '{if ($1 ~ /'"$SampleID"'/) {print $1}}' ${SraRunTable_csv}|cut -d '_' -f 2)
                ...
                ## 3. submit job
                preprocess_sh="${src_url}/${organism}/${SRP}/${SampleID}/${SampleID}_${SRP}_preprocess.sh"
                if test -e "${preprocess_sh}"
                then
                    rm ${preprocess_sh} && echo old_${preprocess_sh}_removed
                    rm ${src_url}/${organism}/${SRP}/${SampleID}/preprocess.* && echo preprocess_qOut_qError_removed
                fi
